...
async def exchange_auth_code_for_token(auth_code: str):
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if user is None:
                ...
                await update_last_time_service_used(token_data["email"])
...
async def verify_jwt_token(request: Request):
    ...
    try:
        ...
        if user is not None:
            await update_last_time_service_used(payload["email"])
        else:
            return
...
async def blacklist_user(user_email: str):
    db = Session()
    try:
...
async def update_last_time_service_used(user_email: str):
    db = Session()
    try:
        db_user = db.query(User).filter(User.email == user_email).first()
        if db_user:
