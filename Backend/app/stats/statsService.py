...
async def generate_stats(user_id: int):
    ...
    try:
        ...
        stats.bookings = sum(
            1 for chat in chats if _has_flag(_parse_chat_history(chat.chat_history), "is_booking")
        )
        stats.human_handoffs = sum(
            1 for chat in chats if _has_flag(_parse_chat_history(chat.chat_history), "is_human_handoff")
        )
...
def count_user_messages(chat_history_str: str) -> int:
    ...
    history = _parse_chat_history(chat_history_str)
    return sum(
